function CGfxButton(iXPos,iYPos,oSprite, oParentContainer){
    ...
    this.pulseAnimation = function () {
        _oTween = createjs.Tween.get(_oButton,{loop:-1}).to({scale: _iScaleFactor*1.1}, 850, createjs.Ease.quadOut).to({scale: _iScaleFactor}, 650, createjs.Ease.quadIn);
    };

    this.trembleAnimation = function () {
        _oTween = createjs.Tween.get(_oButton,{loop:-1}).to({rotation: 5}, 75, createjs.Ease.quadOut).to({rotation: -5}, 140, createjs.Ease.quadIn).to({rotation: 0}, 75, createjs.Ease.quadIn);
    };
